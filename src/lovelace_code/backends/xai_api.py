...
@register_backend("xai")
class XAIBackend(Backend):
    """Uses xAI API (Grok-4, Grok-4.1, etc.)."""
    ...
    def is_available(self) -> bool:
        try:
            import openai  # noqa: F401 - xAI uses OpenAI-compatible API
            return True
        except ImportError:
            return False
    ...
    def generate(self, prompt: str, config: GenerationConfig) -> str:
        ...
        try:
            ...
            raise BackendError(f"xAI API error: {exc}") from exc
    ...
    def stream(self, prompt: str, config: GenerationConfig) -> Iterable[str]:
        ...
        try:
            ...
            raise BackendError(f"xAI API stream error: {exc}") from exc
